...
public class FancyAxis {
	...
	public FancyAxis(Joystick joystick, int axis) {
	...
	public FancyAxis(Joystick joystick, int axis, double deadband, double minPower, double midPower) {
		this.joystick = joystick;
		this.axis = axis;
	...
	public double getValueLinear() {
	    if(Math.abs(joystick.getRawAxis(axis)) < deadband) return 0;
	    else return joystick.getRawAxis(axis);
	}
